...
import com.phantam.moviedesktopapp.Model.Genre;
...
public class TMDbService {
     ...
     */
    public static List<Movie> fetchPopularMovies(int page) {
     ...
     * @return Danh sách Movie
     */
    public static List<Movie> fetchUpComingMovies(int page) {
        List<Movie> movies = new ArrayList<>();
        ExecutorService executor = Executors.newFixedThreadPool(10); // tối đa 10 luồng
        List<Future<Movie>> futureMovies = new ArrayList<>();
     ...
     * @return Danh sách Movie
     */
    public static List<Movie> fetchTopRatedMovies(int page) {
        List<Movie> movies = new ArrayList<>();
        ExecutorService executor = Executors.newFixedThreadPool(10); // tối đa 10 luồng
        List<Future<Movie>> futureMovies = new ArrayList<>();
    ...
    public static List<Movie> fetchMoviesByCategory(MainViewController.MovieCategory category, int page) {
        switch (category) {
            ...
            case TOP_RATED: return fetchTopRatedMovies(page);
            default: return fetchPopularMovies(page);
        }
    }
    ...
    public static List<Genre> fetchGenres() {
        ...
        try {
            URL url = new URL(BASE_URL + "genre/movie/list?api_key=" + API_KEY);
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("GET");
